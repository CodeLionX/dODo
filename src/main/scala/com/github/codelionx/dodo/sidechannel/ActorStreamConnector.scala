...
object ActorStreamConnector {
  ...
  /**
    * Builds a flow for receiving stream elements to the `actorRef` with a
  ...
  def withQueueSource[IN <: AnyRef, OUT <: AnyRef](
                                                    actorRef: ActorRef,
                                                    deserializationClass: Class[_ <: IN]
                                                  )(
  ...
  /**
    ...
    * incoming messages get deserialized to type `IN` and sent to the actor.
    * The outgoing message `singleElem` is directly deserialized to a
  ...
  private def buildFlow[IN <: AnyRef, OUT <: AnyRef, MAT](
    ...
    val sinkSourceFlow = Flow.fromSinkAndSourceMat(
      Sink.actorRefWithAck[IN](actorRef, StreamInit, StreamACK, StreamComplete),
      source
    )(Keep.right)
    ...
    Flow[ByteString]
      ...
      // .reduce(_ ++ _)
      .map(bytes => serialization.deserialize(bytes.toArray, deserializationClass))
      .map {
